...
router.get('/apuracao', (req, res) => {
  try {
    ...
    const cfgRow            = db.prepare(`SELECT valor FROM configuracoes WHERE chave = ?`).get(cfgKey);
...
router.get('/relatorio', (req, res) => {
  try {
    ...
    const cfgRow            = db.prepare(`SELECT valor FROM configuracoes WHERE chave = ?`).get(`inss_dctfweb_${p.novo}`);
    const dctfweb_declarado = cfgRow ? parseFloat(cfgRow.valor) || 0 : 0;
