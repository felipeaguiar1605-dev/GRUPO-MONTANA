...
app.use('/api/ofx',          require('./ofx'));

// ─── Importação Alterdata (funcionários/folha) ───────────────────
app.use('/api/alterdata',    require('./alterdata'));

// ─── Módulos extras (melhorias 1-6) ──────────────────────────────
app.use('/api/certidoes',    require('./certidoes'));
app.use('/api/licitacoes',   require('./licitacoes'));
app.use('/api/calculadora',  require('./calculadora'));
app.use('/api/dre',          require('./dre'));
app.use('/api/notificacoes', require('./notificacoes'));

// ─── Módulo Boletins de Medição ─────────────────────────────────
app.use('/api/boletins',     require('./boletins'));

// ─── Módulo RH / Departamento Pessoal ───────────────────────────
app.use('/api/rh',           require('./rh'));

// ─── Módulo Ponto Eletrônico ─────────────────────────────────────
app.use('/api/ponto',        require('./ponto'));


// ─── Módulo WebISS (NFS-e Palmas-TO) ────────────────────────────
app.use('/api/webiss',       require('./webiss'));

// ─── Módulo Transparência Palmas (conciliação com portal) ────────
app.use('/api/transparencia', require('./transparencia'));

// ─── Módulo Conciliação Robusta (pagador_alias + sugestões) ─────
app.use('/api/conciliacao-robusta', require('./conciliacao-robusta'));

// ─── Módulo Assistente IA (Claude) ──────────────────────────────
app.use('/api/ia',           require('./ia'));

// ─── Módulo Google Drive + IA ────────────────────────────────────
app.use('/api/drive',        require('./drive'));

// ─── Módulo Estoque (equipamentos, EPIs, consumíveis) ───────────
app.use('/api/estoque',      require('./estoque'));

// ─── Módulo Sync Banco do Brasil ────────────────────────────────
app.use('/api/bb',           require('./bb-sync'));

// ─── Módulo Volus (Vale Alimentação / Benefícios) ────────────
app.use('/api/volus',        require('./volus'));

// ─── Módulo Jurídico ─────────────────────────────────────────
app.use('/api/juridico',     require('./juridico'));

// ─── Apuração PIS/COFINS Segurança ───────────────────────────
app.use('/api/piscofins-seguranca', require('./piscofins-seguranca'));

// ─── INSS Retido / S-1300 (Assessoria) ───────────────────────
app.use('/api/inss-retido', require('./inss-retido'));

// ─── Painel de Pagamentos por Contrato ───────────────────────
app.use('/api/pagamentos-contrato', require('./pagamentos-contrato'));
...
app.use('/api/usuarios',     require('./usuarios').router);
...
app.get('/api/consolidado', require('../auth').authMiddleware, async (req, res) => {
  try {
    const resultado = {};
    ...
    for (const [key, company] of Object.entries(COMPANIES)) {
      try {
        ...
        const extratos = await db.prepare(`SELECT COUNT(*) cnt, COALESCE(SUM(credito),0) entradas, COALESCE(SUM(debito),0) saidas FROM extratos WHERE data_iso>=? AND data_iso<=?`).get(from, to);
        const nfs      = await db.prepare(`SELECT COUNT(*) cnt, COALESCE(SUM(valor_bruto),0) bruto FROM notas_fiscais WHERE (data_emissao>=? AND data_emissao<=?) OR (data_emissao='' AND created_at>=? AND created_at<=?)`).get(from, to, from, to);
        const desp     = await db.prepare(`SELECT COALESCE(SUM(valor_bruto),0) total FROM despesas WHERE data_iso>=? AND data_iso<=?`).get(from, to);
...
try {
  ...
  const { enviarAlertasEmpresa, verificarDuplicatas, enviarAlertaDedup } = require('./notificacoes');
  ...
  cron.schedule('0 6 * * *', async () => {
    ...
    for (const key of Object.keys(COMPANIES)) {
      try {
        ...
        const db  = getDb(key);
        const cfg = (await db.prepare(`SELECT chave, valor FROM configuracoes WHERE chave LIKE 'bb_%'`).all()).reduce((acc, r) => { acc[r.chave.replace('bb_', '')] = r.valor; return acc; }, {});
  ...
  cron.schedule('0 6 1 * *', async () => {
    ...
    for (const [key] of Object.entries(COMPANIES)) {
      try {
        ...
        try {
          const { enviarAlertasEmpresa } = require('./notificacoes');
