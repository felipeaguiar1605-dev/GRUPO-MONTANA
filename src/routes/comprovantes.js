...
router.post('/:id/vincular', async (req, res) => {
  ...
  try {
    ...
    await recomputarStatus(db, cp.id);

    // Fase C: se vínculo for em NF, recalcula status + retencao_efetiva da NF
    let nfStatus = null;
    if (tipo_destino === 'NF') {
      try { nfStatus = await recalcularNF(db, destino_id); } catch (_) {}
    }
...
router.delete('/vinculos/:vid', async (req, res) => {
  try {
    ...
    await recomputarStatus(req.db, v.comprovante_id);
    // Fase C: se era NF, recalcula status derivado
    if (v.tipo_destino === 'NF') {
      try { await recalcularNF(req.db, v.destino_id); } catch (_) {}
    }
    res.json({ ok: true });
  } catch (e) {
    res.status(500).json({ error: e.message });
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    for (const r of nfDest) {
      try { await recalcularNF(req.db, r.destino_id); } catch (_) {}
    }
    if (cp.arquivo_path) {
      const abs = path.join(__dirname, '..', '..', cp.arquivo_path);
