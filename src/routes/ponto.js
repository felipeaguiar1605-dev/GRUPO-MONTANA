...
router.post('/importar', async (req, res) => {
  try {
    ...
    if (lote.length > 0) await insert(lote);
  ...
  try {
    ...
    const upsertItem = d.transaction(async () => {
      for (const func of funcionarios) {
        ...
        const existing = await d.prepare(`SELECT id FROM rh_folha_itens WHERE folha_id = ? AND funcionario_id = ?`).get(folha.id, func.id);
        if (existing) {
          await d.prepare(`
      ...
      await d.prepare(`UPDATE rh_folha SET total_bruto=?,total_descontos=?,total_liquido=?,status='RASCUNHO' WHERE id=?`)
