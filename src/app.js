...
function renderExtFilters(){
  ...
  const botoesHtml = mesesBtn
    ...
    .map(m => `<button onclick="_extMes='${m}';_extPage=1;loadExtratos()" style="${btnStyle(_extMes===m)}">${MES_LABEL[m]||m}/${ano}</button>`)
    .join('');
  ...
  document.getElementById('ext-filters').innerHTML=`
    <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap">
      ...
      <button onclick="_extMes='';_extPage=1;loadExtratos()" style="${btnStyle(!_extMes)}">Todos</button>
      ${botoesHtml}
...
async function loadExtratos(){
  ...
  if(mesesData.meses && mesesData.meses.length) _extMesesDisponiveis = mesesData.meses;
