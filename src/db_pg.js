...
// Type parsers: PG returns BIGINT (OID 20) and NUMERIC (OID 1700) as strings
// Parse them as JS numbers for drop-in SQLite compatibility
types.setTypeParser(20, (val) => val === null ? null : Number(val));    // BIGINT → Number
...
function convertSql(sql) {
  return sql
    ...
    // strftime handled by fixStrftime() in convertSqlFull() — do NOT partial-replace here
    ...
    // date('now') → to_char(CURRENT_DATE, 'YYYY-MM-DD') para comparação com texto
    .replace(/date\('now'\)/gi, "to_char(CURRENT_DATE, 'YYYY-MM-DD')")
    // julianday(A) - julianday(B) → (A::date - B::date) — SQLite date arithmetic
    .replace(/julianday\(((?:[^)(]|\([^)]*\))*)\)\s*-\s*julianday\(((?:[^)(]|\([^)]*\))*)\)/gi, '($1::date - $2::date)')
    .replace(/julianday\(((?:[^)(]|\([^)]*\))*)\)/gi, '($1::date)')
    // Trim extra spaces
    .replace(/\s{2,}/g, ' ')
    // Quote camelCase column names (PG is case-sensitive)
    .replace(/numContrato/g, '"numContrato"')
    .trim();
}
