...
router.get('/resumo', (req, res) => {
  try {
    ...
    res.json({
      resumo: { oficiosPendentes, oficiosVencidos, oficiosVencendo, processosAtivos, audienciasEm30, valorRiscoTotal, riscosAltos, riscosTotal },
      alertas_oficios: alertasOficios,
      audiencias_proximas: audiencias,
      riscos_altos: riscos
    });
...
router.get('/oficios', (req, res) => {
  try {
    ...
    let sql = `SELECT * FROM jur_oficios WHERE 1=1`;
    const p = [];
...
router.post('/oficios', (req, res) => {
  try {
    ...
    `).run(numero||null, tipo||'RECEBIDO', orgao, assunto, contrato_ref||null,
           data_recebimento||null, data_prazo||null, responsavel||null,
           prioridade||'NORMAL', valor_risco||null, obs||null);
    res.json({ id: r.lastInsertRowid, message: 'Ofício registrado' });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.put('/oficios/:id', (req, res) => {
  try {
    ...
    `).run(numero||null, tipo||'RECEBIDO', orgao, assunto, contrato_ref||null,
           ...
           status||'PENDENTE', prioridade||'NORMAL', valor_risco||null, obs||null, req.params.id);
    res.json({ message: 'Ofício atualizado' });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
    ...
    const r = db.prepare(`
      INSERT INTO jur_processos
        (numero,tipo,subtipo,orgao,vara,assunto,contrato_ref,autor,reu,
         data_abertura,data_prazo,proxima_audiencia,advogado,fase,
         valor_causa,valor_risco,probabilidade,obs)
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    `).run(numero||null, tipo, subtipo||null, orgao, vara||null, assunto,
           contrato_ref||null, autor||null, reu||null,
           data_abertura||null, data_prazo||null, proxima_audiencia||null,
           advogado||null, fase||null,
           valor_causa||null, valor_risco||null, probabilidade||'MEDIA', obs||null);
    res.json({ id: r.lastInsertRowid, message: 'Processo cadastrado' });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.put('/processos/:id', (req, res) => {
  try {
    ...
    `).run(numero||null, tipo, subtipo||null, orgao, vara||null, assunto,
           contrato_ref||null, autor||null, reu||null,
           data_abertura||null, data_prazo||null, proxima_audiencia||null,
           advogado||null, status||'ATIVO', fase||null,
           valor_causa||null, valor_risco||null, probabilidade||'MEDIA', obs||null,
           req.params.id);
...
router.get('/riscos', (req, res) => {
  try {
    ...
    const p = [];
    if (contrato_ref)  { sql += ` AND contrato_ref = ?`; p.push(contrato_ref); }
    if (probabilidade) { sql += ` AND probabilidade = ?`; p.push(probabilidade); }
    if (status)        { sql += ` AND status = ?`; p.push(status); }
...
router.post('/riscos', (req, res) => {
  try {
    ...
    `).run(contrato_ref, tipo, descricao, probabilidade||'MEDIA', impacto||'MEDIO',
           valor_estimado||null, mitigacao||null, responsavel||null,
           data_identificacao||new Date().toISOString().split('T')[0], obs||null);
    res.json({ id: r.lastInsertRowid, message: 'Risco registrado' });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.put('/riscos/:id', (req, res) => {
  try {
    ...
    `).run(contrato_ref, tipo, descricao, probabilidade||'MEDIA', impacto||'MEDIO',
           ...
           responsavel||null, data_revisao||null, obs||null, req.params.id);
